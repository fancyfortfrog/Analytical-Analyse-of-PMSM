...
% Br=1.11;          % PM的剩磁/T，有限元中使用钕铁硼材料的剩磁密度
Br=0.6;
...
for num_phase= 1:12
    for ti = 1:1000
        t=t_region(1,ti);
        if ismember(num_phase,[1,2,3,4])
            I_phase(ti,num_phase)=I_phase_max*sin(we*t+theta0-(num_phase-1)*pi/12);
        elseif ismember(num_phase,[5,6,7,8])
            I_phase(ti,num_phase)=I_phase_max*sin(we*t+theta0-(num_phase-5)*pi/12-2*pi/3);
        elseif ismember(num_phase,[9,10,11,12])
            I_phase(ti,num_phase)=I_phase_max*sin(we*t+theta0-(num_phase-9)*pi/12+2*pi/3);
        end
    end
end
...
I_phase=I_phase(1,:)';
% J_phase=I_phase./S_slot;

S_slot_up=(bsa/2)*(Rsb^2-Rsm^2);     % 上层槽的面积
S_slot_down=(bsa/2)*(Rsm^2-Rt^2);    % 下层槽的面积

It1=[I_phase(2,1) ; I_phase(3,1) ; I_phase(4,1) ; -I_phase(9,1) ; -I_phase(10,1) ; -I_phase(11,1) ; -I_phase(12,1) ; I_phase(5,1) ; I_phase(6,1) ; I_phase(7,1) ; I_phase(8,1) ; -I_phase(1,1)];
It1=[It1;-It1;It1;-It1];             % 下层绕组的排布

It2=[I_phase(1,1) ; I_phase(2,1) ; I_phase(3,1) ; I_phase(4,1) ; -I_phase(9,1) ; -I_phase(10,1) ; -I_phase(11,1) ; -I_phase(12,1) ; I_phase(5,1) ; I_phase(6,1) ; I_phase(7,1) ; I_phase(8,1)];
It2=[It2;-It2;It2;-It2];             % 上层绕组的排布
...
filename1="H:\multi_phase_shift_angle\differente_Br\FEA_Br_15_0.8.csv";
filename2="H:\multi_phase_shift_angle\differente_Br\FEA_Bt_15_0.8.csv";
...
filename3="H:\multi_phase_shift_angle\differente_Br\FEA_Fr_15_0.8.csv";
filename4="H:\multi_phase_shift_angle\differente_Br\FEA_Ft_15_0.8.csv";
